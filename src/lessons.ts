...
export const LESSONS: Lesson[] = [
  {
    ...
    demo: { prompt: 'Once upon a time, a small robot discovered', target: 'tokens', title: 'Look at the separate pieces', callout: 'Each highlighted piece was generated separately. Spaces belong to tokens too, even though the finished sentence reads smoothly.', steps: 4, seed: 42 },
  },
  {
    ...
    demo: { prompt: 'Once upon a time, a small robot discovered', target: 'probabilities', title: 'These odds belong to one moment', callout: 'This ranked list was calculated from the prompt and selected branch. Choose another token and the model must calculate a new list.', steps: 1, seed: 42 },
  },
  {
  ...
  },
  {
    ...
    demo: { prompt: 'Once upon a time, a small robot discovered', target: 'tree', title: 'One prompt, two possible paths', callout: 'Both branches were available at the same moment. Ordinary chat keeps one and hides the other; this tree preserves both.', steps: 1, branches: 2, seed: 42 },
  },
  {
    id: 'early-choices',
    ...
    demo: { prompt: 'The child looked inside the mysterious box and found', target: 'tree', title: 'The split changes what follows', callout: 'These paths share one prompt, then take different early tokens. Each path gives the model a different context for its next prediction.', steps: 2, branches: 2, seed: 42 },
  },
  {
    id: 'temperature',
